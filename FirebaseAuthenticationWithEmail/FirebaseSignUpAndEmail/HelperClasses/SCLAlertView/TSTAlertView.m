...
#import "TSTAlertViewStyleKit.h"
#import "UIImage+ImageEffects.h"
#import "TSTTimerDisplay.h"
#import "TSTMacros.h"
...
@interface TSTAlertView ()  <UITextFieldDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) UIWindow *TSTAlertWindow;
@property (nonatomic, copy) TSTDismissBlock dismissBlock;
...
- (void)setupViewWindowWidth:(CGFloat)windowWidth
{
    ...
    _labelTitle.textColor = UIColorFromHEX(0xE96D1A); //Dark Grey
    _viewText.textColor = UIColorFromHEX(0x4D4D4D); //Dark Grey
    _contentView.layer.borderColor = UIColorFromHEX(0xE96D1A).CGColor; //Light Grey
}
...
- (void)setupNewWindow
{
    ...
    self.TSTAlertWindow = alertWindow;
...
- (void)viewWillLayoutSubviews
{
    ...
    for (TSTTextView *textField in _inputs)
    {
...
- (void)handleTap:(UITapGestureRecognizer *)gesture
{
    if (_shouldDismissOnTapOutside)
    {
        ...
        for(TSTTextView *txt in _inputs)
        {
...
- (void)setShouldDismissOnTapOutside:(BOOL)shouldDismissOnTapOutside
{
    ...
    if(_shouldDismissOnTapOutside)
    {
        ...
        [_usingNewWindow ? _TSTAlertWindow : _backgroundView addGestureRecognizer:_gestureRecognizer];
    }
}
...
{
    // Add switch view
    TSTSwitchView *switchView = [[TSTSwitchView alloc] initWithFrame:CGRectMake(0, 0, self.windowWidth, 31.0f)];
...
- (TSTTextView *)addTextField:(NSString *)title
{
    ...
    // Add text field
    TSTTextView *txt = [[TSTTextView alloc] init];
    ...
    if (_inputs.count > 1)
    {
        ...
        TSTTextView *priorField = _inputs[indexOfCurrentField - 1];
...
//    UIColor *orangeColorCode = [UIColor colorWithRed:233.0/255.0 green:109.0/255.0 blue:26.0/255.0 alpha:1.0];
    ...
    UIColor *viewColor = UIColorFromHEX(0xE96D1A);
...
- (TSTButton *)addDoneButtonWithTitle:(NSString *)title
{
    TSTButton *btn = [self addButton:title];
...
{
    TSTButton *btn = [self addButton:title actionBlock:action];
...
- (void)buttonTapped:(TSTButton *)btn
{
    ...
    if (btn.actionType == TSTBlock)
    {
        if (btn.actionBlock)
            btn.actionBlock();
    }
    else if (btn.actionType == TSTSelector)
    {
...
- (void)addTimerToButtonIndex:(NSInteger)buttonIndex reverse:(BOOL)reverse
{
    ...
    buttonTimer = [[TSTTimerDisplay alloc] initWithOrigin:CGPointMake(5, 5) radius:13 lineWidth:4];
...
- (TSTAlertViewResponder *)showTitle:(UIViewController *)vc image:(UIImage *)image color:(UIColor *)color title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(TSTAlertViewStyle)style
{
    if(_usingNewWindow)
    {
        // Save previous window
        self.previousWindow = [UIApplication sharedApplication].keyWindow;
        self.backgroundView.frame = _TSTAlertWindow.bounds;
        
        // Add window subview
        [_TSTAlertWindow addSubview:_backgroundView];
    }
    else
    {
    ...
    {
        case Success:
            viewColor = UIColorFromHEX(0x0E1E4E);
            iconImage = TSTAlertViewStyleKit.imageOfCheckmark;
            break;
            
        case Error:
            viewColor = UIColorFromHEX(0x0E1E4E);
            iconImage = TSTAlertViewStyleKit.imageOfCross;
            break;
            
        case Notice:
            viewColor = UIColorFromHEX(0x727375);
            iconImage = TSTAlertViewStyleKit.imageOfNotice;
            break;
            
        case Warning:
            viewColor = UIColorFromHEX(0xFFD110);
            iconImage = TSTAlertViewStyleKit.imageOfWarning;
            break;
            
        case Info:
            viewColor = UIColorFromHEX(0x2866BF);
            iconImage = TSTAlertViewStyleKit.imageOfInfo;
            break;
            
        case Edit:
            viewColor = UIColorFromHEX(0xA429FF);
            iconImage = TSTAlertViewStyleKit.imageOfEdit;
            break;
        ...
        case Question:
            viewColor = UIColorFromHEX(0x727375);
            iconImage = TSTAlertViewStyleKit.imageOfQuestion;
            break;
    ...
    if (duration > 0)
    {
        ...
        {
            TSTButton *btn = _buttons[buttonTimer.buttonIndex];
    ...
    if(_usingNewWindow)
    {
        [_TSTAlertWindow makeKeyAndVisible];
    }
    ...
    // Chainable objects
    return [[TSTAlertViewResponder alloc] init:self];
}
...
- (TSTForceHideBlock)forceHideBlock:(TSTForceHideBlock)forceHideBlock
{
...
- (void)fadeOut
{
    [UIView animateWithDuration:0.3f animations:^{
        ...
        if (_usingNewWindow)
        {
            ...
            [self.TSTAlertWindow setHidden:YES];
            self.TSTAlertWindow = nil;
        }
        else
        {
...
@interface TSTALertViewTextFieldBuilder()
#pragma mark - Parameters
@property(copy, nonatomic) NSString *parameterTitle;

#pragma mark - Available later after adding
@property(weak, nonatomic) TSTTextView *textField;

#pragma mark - Setters
@property(copy, nonatomic) TSTALertViewTextFieldBuilder *(^title) (NSString *title);
@end

@implementation TSTALertViewTextFieldBuilder
- (TSTALertViewTextFieldBuilder *(^) (NSString *title))title {
    if (!_title) {
...
@interface TSTALertViewButtonBuilder()
...
#pragma mark - Available later after adding
@property(weak, nonatomic) TSTButton *button;

#pragma mark - Setters
@property(copy, nonatomic) TSTALertViewButtonBuilder *(^title) (NSString *title);
@property(copy, nonatomic) TSTALertViewButtonBuilder *(^target) (id target);
@property(copy, nonatomic) TSTALertViewButtonBuilder *(^selector) (SEL selector);
@property(copy, nonatomic) TSTALertViewButtonBuilder *(^actionBlock) (void(^actionBlock)(void));
@property(copy, nonatomic) TSTALertViewButtonBuilder *(^validationBlock) (BOOL(^validationBlock)(void));

@end

@implementation TSTALertViewButtonBuilder
- (TSTALertViewButtonBuilder *(^) (NSString *title))title {
    if (!_title) {
        __weak typeof(self) weakSelf = self;
        _title = ^(NSString *title){
            weakSelf.parameterTitle = title;
...
- (TSTALertViewButtonBuilder *(^) (id target))target {
    if (!_target) {
        __weak typeof(self) weakSelf = self;
        _target = ^(id target){
...
- (TSTALertViewButtonBuilder *(^) (SEL selector))selector {
    if (!_selector) {
        __weak typeof(self) weakSelf = self;
...
- (TSTALertViewButtonBuilder *(^) (void(^actionBlock)(void)))actionBlock {
    if (!_actionBlock) {
        __weak typeof(self) weakSelf = self;
...
- (TSTALertViewButtonBuilder *(^) (BOOL(^validationBlock)(void)))validationBlock {
    if (!_validationBlock) {
        __weak typeof(self) weakSelf = self;
...
@interface TSTAlertViewBuilder()

@property (strong, nonatomic) TSTAlertView *alertView;

@end

@implementation TSTAlertViewBuilder
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.alertView = [[TSTAlertView alloc] init];
    }
    return self;
}
- (instancetype)initWithNewWindow {
    self = [super init];
    if (self) {
        self.alertView = [[TSTAlertView alloc] initWithNewWindow];
    }
    return self;
}
...
- (instancetype)initWithNewWindowWidth:(CGFloat)width {
    self = [super init];
    if (self) {
        self.alertView = [[TSTAlertView alloc] initWithNewWindowWidth:width];
    }
    return self;
}

#pragma mark - Properties
- (TSTAlertViewBuilder *(^) (CGFloat cornerRadius))cornerRadius {
    if (!_cornerRadius) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (BOOL tintTopCircle))tintTopCircle {
    if (!_tintTopCircle) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (BOOL useLargerIcon))useLargerIcon {
    if (!_useLargerIcon) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UILabel *labelTitle))labelTitle {
    if (!_labelTitle) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UITextView *viewText))viewText {
    if (!_viewText) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UIActivityIndicatorView *activityIndicatorView))activityIndicatorView {
    if (!_activityIndicatorView) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (BOOL shouldDismissOnTapOutside))shouldDismissOnTapOutside {
    if (!_shouldDismissOnTapOutside) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (NSURL *soundURL))soundURL {
    if (!_soundURL) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (TSTAttributedFormatBlock attributedFormatBlock))attributedFormatBlock {
    if (!_attributedFormatBlock) {
        __weak typeof(self) weakSelf = self;
        _attributedFormatBlock = ^(TSTAttributedFormatBlock attributedFormatBlock) {
...
- (TSTAlertViewBuilder *(^) (CompleteButtonFormatBlock completeButtonFormatBlock))completeButtonFormatBlock {
...
- (TSTAlertViewBuilder *(^) (ButtonFormatBlock buttonFormatBlock))buttonFormatBlock {
...
- (TSTAlertViewBuilder *(^) (TSTForceHideBlock forceHideBlock))forceHideBlock {
    if (!_forceHideBlock) {
        __weak typeof(self) weakSelf = self;
        _forceHideBlock = ^(TSTForceHideBlock forceHideBlock) {
...
- (TSTAlertViewBuilder *(^) (TSTAlertViewHideAnimation hideAnimationType))hideAnimationType {
    if (!_hideAnimationType) {
        __weak typeof(self) weakSelf = self;
        _hideAnimationType = ^(TSTAlertViewHideAnimation hideAnimationType) {
...
- (TSTAlertViewBuilder *(^) (TSTAlertViewShowAnimation showAnimationType))showAnimationType {
    if (!_showAnimationType) {
        __weak typeof(self) weakSelf = self;
        _showAnimationType = ^(TSTAlertViewShowAnimation showAnimationType) {
...
- (TSTAlertViewBuilder *(^) (TSTAlertViewBackground backgroundType))backgroundType {
    if (!_backgroundType) {
        __weak typeof(self) weakSelf = self;
        _backgroundType = ^(TSTAlertViewBackground backgroundType) {
...
- (TSTAlertViewBuilder *(^) (UIColor *customViewColor))customViewColor {
    if (!_customViewColor) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UIColor *backgroundViewColor))backgroundViewColor {
...
- (TSTAlertViewBuilder *(^) (UIColor *iconTintColor))iconTintColor {
    if (!_iconTintColor) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (CGFloat circleIconHeight))circleIconHeight {
...
- (TSTAlertViewBuilder *(^) (CGRect extensionBounds))extensionBounds {
    if (!_extensionBounds) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (BOOL statusBarHidden))statusBarHidden {
    if (!_statusBarHidden) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UIStatusBarStyle statusBarStyle))statusBarStyle {
    if (!_statusBarStyle) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (TSTDismissBlock dismissBlock))alertIsDismissed {
    if (!_alertIsDismissed) {
        __weak typeof(self) weakSelf = self;
        _alertIsDismissed = ^(TSTDismissBlock dismissBlock) {
...
- (TSTAlertViewBuilder *(^) (void))removeTopCircle {
    if (!_removeTopCircle) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UIView *view))addCustomView {
    if (!_addCustomView) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (NSString *title))addTextField {
    if (!_addTextField) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewBuilder *(^) (UITextField *textField))addCustomTextField {
...
- (TSTAlertViewBuilder *(^) (NSString *title))addSwitchViewWithLabelTitle {
...
- (TSTAlertViewBuilder *(^) (NSInteger buttonIndex, BOOL reverse))addTimerToButtonIndex {
...
- (TSTAlertViewBuilder *(^) (NSString *titleFontFamily, CGFloat size))setTitleFontFamily {
...
- (TSTAlertViewBuilder *(^) (NSString *bodyTextFontFamily, CGFloat size))setBodyTextFontFamily {
...
- (TSTAlertViewBuilder *(^) (NSString *buttonsFontFamily, CGFloat size))setButtonsTextFontFamily {
...
- (TSTAlertViewBuilder *(^) (NSString *title, TSTActionBlock action))addButtonWithActionBlock {
    if (!_addButtonWithActionBlock) {
        __weak typeof(self) weakSelf = self;
        _addButtonWithActionBlock = ^(NSString *title, TSTActionBlock action) {
...
- (TSTAlertViewBuilder *(^) (NSString *title, TSTValidationBlock validationBlock, TSTActionBlock action))addButtonWithValidationBlock {
    if (!_addButtonWithValidationBlock) {
        __weak typeof(self) weakSelf = self;
        _addButtonWithValidationBlock = ^(NSString *title, TSTValidationBlock validationBlock, TSTActionBlock action) {
...
- (TSTAlertViewBuilder *(^) (NSString *title, id target, SEL selector))addButtonWithTarget {
...
- (TSTAlertViewBuilder *(^)(TSTALertViewButtonBuilder *builder))addButtonWithBuilder {
    if (!_addButtonWithBuilder) {
        __weak typeof(self) weakSelf = self;
        _addButtonWithBuilder = ^(TSTALertViewButtonBuilder *builder){
            TSTButton *button = nil;
...
- (TSTAlertViewBuilder *(^)(TSTALertViewTextFieldBuilder *builder))addTextFieldWithBuilder {
    if (!_addTextFieldWithBuilder) {
        __weak typeof(self) weakSelf = self;
        _addTextFieldWithBuilder = ^(TSTALertViewTextFieldBuilder *builder){
...
@interface TSTAlertViewShowBuilder()
...
@property(assign, nonatomic) TSTAlertViewStyle parameterStyle;
@property(assign, nonatomic) NSTimeInterval parameterDuration;

#pragma mark - Setters
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^viewController)(UIViewController *viewController);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^image)(UIImage *image);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^color)(UIColor *color);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^title)(NSString *title);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^subTitle)(NSString *subTitle);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^completeText)(NSString *completeText);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^style)(TSTAlertViewStyle style);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^closeButtonTitle)(NSString *closeButtonTitle);
@property(copy, nonatomic) TSTAlertViewShowBuilder *(^duration)(NSTimeInterval duration);

#pragma mark - Show
@property(copy, nonatomic) void (^show)(TSTAlertView *view, UIViewController *controller);
@end

@implementation TSTAlertViewShowBuilder


#pragma mark - Setters
- (TSTAlertViewShowBuilder *(^)(UIViewController *viewController))viewController {
    if (!_viewController) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewShowBuilder *(^)(UIImage *image))image {
    if (!_image) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewShowBuilder *(^)(UIColor *color))color {
    if (!_color) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewShowBuilder *(^)(NSString *title))title {
    if (!_title) {
        __weak typeof(self) weakSelf = self;
        _title = ^(NSString *title){
            weakSelf.parameterTitle = title;
...
- (TSTAlertViewShowBuilder *(^)(NSString *subTitle))subTitle {
    if (!_subTitle) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewShowBuilder *(^)(NSString *completeText))completeText {
    if (!_completeText) {
        __weak typeof(self) weakSelf = self;
...
- (TSTAlertViewShowBuilder *(^)(TSTAlertViewStyle style))style {
    if (!_style) {
        __weak typeof(self) weakSelf = self;
        _style = ^(TSTAlertViewStyle style){
...
- (TSTAlertViewShowBuilder *(^)(NSString *closeButtonTitle))closeButtonTitle {
...
- (TSTAlertViewShowBuilder *(^)(NSTimeInterval duration))duration {
    if (!_duration) {
        __weak typeof(self) weakSelf = self;
...
- (void (^)(TSTAlertView *view, UIViewController *controller))show {
    if (!_show) {
        __weak typeof(self) weakSelf = self;
        _show = ^(TSTAlertView *view, UIViewController *controller) {
